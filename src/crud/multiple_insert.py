...
def multiple_insert(settings):
    ...
    # получение данных
    data = get_data_from_api(settings.URL, settings.NUM_PAGE)
    ...
    for i in range(settings.NUM_PAGE):
        ...
        try:
            ...
            insert_data(data=invalid_data, scheme_name=settings.SCHEME)
        ...
        user_id = insert_data(data=users_data, scheme_name=settings.SCHEME)
        logger.debug(f'Inserted data successfully for user {user_id}')

        contact_data = get_contact_details(valid_data, user_id)
        insert_data(data=contact_data, scheme_name=settings.SCHEME)

        media_data = get_media_data(valid_data, user_id)
        insert_data(data=media_data, scheme_name=settings.SCHEME)

        registration_data = get_registration_data(valid_data, user_id)
        insert_data(data=registration_data, scheme_name=settings.SCHEME)
